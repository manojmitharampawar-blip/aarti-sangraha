...
function SearchContent() {
  ...
  return (
    <div className="space-y-5">
      ...
      {isAiMode && (
        <div className="space-y-2 p-3 rounded-2xl bg-saffron-500/5 dark:bg-saffron-950/20 border border-saffron-500/20">
          <div className="flex items-center gap-1.5 text-xs font-bold text-saffron-800 dark:text-saffron-300">
            ...
            <span>
              {isDevanagari
                ? 'माझ्या भावनेनुसार/गरजेनुसार स्तोत्रे शोधा (Spiritual Intent):'
                : 'Intent / Spiritual Need:'}
            </span>
          </div>
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none text-xs">
            ...
            {DEVOTIONAL_INTENT_CATEGORIES.map(cat => (
              <button
                key={cat.key}
                onClick={() => setSelectedIntent(cat.key)}
                className={`px-3 py-1.5 rounded-full font-bold shrink-0 transition-all ${
                  selectedIntent === cat.key
                    ? 'bg-saffron-600 text-white shadow-xs'
                    : 'border border-saffron-500/30 bg-[var(--card-main)] text-[var(--text-secondary)] hover:border-saffron-500'
                }`}
              >
                <span>{cat.icon}</span>{' '}
                <span>{isDevanagari ? cat.labelMr : cat.labelEn}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      {/* Popular Trending Prayers Chips */}
      {!query && (
        <div className="space-y-2">
          <div className="flex items-center gap-1 text-xs font-bold text-[var(--text-secondary)]">
            ...
            <span>{isDevanagari ? 'नेहमी शोधल्या जाणाऱ्या आरत्या' : 'Frequently Searched:'}</span>
          </div>
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none text-xs">
            {POPULAR_SEARCH_CHIPS.map(chip => (
              <button
                ...
                className="px-2.5 py-1 rounded-lg border border-[var(--border-main)] bg-[var(--card-main)] text-[var(--text-secondary)] hover:text-saffron-600 hover:border-saffron-500/50 shrink-0 font-medium transition-colors"
              >
                {chip}
              </button>
            ))}
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-1.5 text-xs font-bold text-[var(--text-secondary)] uppercase">
          ...
          <span>{isDevanagari ? 'देवता (Deity)' : 'Filter by Deity'}</span>
        </div>
        <div className="flex items-center gap-1.5 overflow-x-auto pb-1 scrollbar-none text-xs">
          <button
